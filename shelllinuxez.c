...
// Para compilar: gcc shell.c -o exe -lreadline && ./exe
...
void procuraComando(char** comando) {
    if(strcmp(comando[0], "exit") == 0) { // Compara o que foi digitado com "exit"
		...
		char* user;
        user = getenv("USER");
        printf("\nOla > %s < seja Bem Vindo a nossa aplicação Linux!!! \n", user);
	}
	else if(strcmp(comando[0], "cd") == 0) { // Compara o que foi digitado com "cd"
		chdir(comando[1]);
	}
    else { // Se o comando não exite na nossa shell, tenta executar pelo linux
        system(comando[0]);
    }
}
